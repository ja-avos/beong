...
def filter_vol(vols, filters):
    ...
    if filters['duramin'] != '' or filters['duramax'] != '':
        ...
        for v in vols:
            durS = v.duracion
            if filters['duramin'] != '' and filters['duramax'] == '' and convertDuration(durS) >= convertDuration(filters['duramin']):
                volsd.append(v)
            elif filters['duramin'] == '' and filters['duramax'] != '' and convertDuration(durS) <= convertDuration(filters['duramax']):
                volsd.append(v)
            elif filters['duramin'] != '' and filters['duramax'] != '' and convertDuration(durS) >= convertDuration(filters['duramin']) and convertDuration(durS) <= convertDuration(filters['duramax']):
                volsd.append(v)
        return volsd
    return vols
...
def apply_volunteer(request):
    ...
    if request.method == 'POST':
        ...
        send_email('javelino2311@gmail.com', 'Aplicación Exitosa' + body['saludo'], "Aplicación", loader.render_to_string('mail/apply_mail.html', {'voluntariado': 'Prueba'}))
